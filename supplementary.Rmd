...
formula <- bf(rating ~ group2 * oldnew * test +
  (1 + oldnew | ID)) +
...
fit_2_groups_alt <- brm(formula,
  ...
  chains = 4, iter = 2000, cores = 4,
  backend = "cmdstanr",
  file = here::here("models/fit_2_groups_alt"),
  save_model = "stancode/fit_2_groups_alt.stan"
) |>
  add_criterion("loo")
```
...
```{r}
int_only_samples <- as_draws_df(fit_2_groups_alt, inc_warmup = TRUE) |>
...
formula <- bf(rating ~ group4 * oldnew * test +
  (1 + oldnew | ID)) +
  lf(disc ~ 0 + group4 * test +
    (1 | ID), cmc = FALSE)

fit_4_groups_2 <- brm(formula,
  family = cumulative("logit"),
  data = d,
  prior = priors,
  init = rep(list(inits), 4),
  ...
  chains = 4, iter = 2000, cores = 4,
  backend = "cmdstanr",
  file = here::here("models/fit_4_groups_2"),
  save_model = here::here("stancode/fit_4_groups_2.stan")
) |>
  add_criterion("loo")
```
...
fit_4_groups_3 <- brm(formula,
  ...
  chains = 4, iter = 2000, cores = 4,
  backend = "cmdstanr",
  file = here::here("models/fit_4_groups_3"),
  save_model = here::here("stancode/fit_4_groups_3.stan")
) |>
  add_criterion("loo")
```
