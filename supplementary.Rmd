...
point_estimates |>
  ...
  labs(x = "Test", y = "Learning score", color = "Group") +
  theme_clean()
```
...
d %>%
  ...
  facet_wrap(~group4) +
  scale_color_viridis_d(direction = 1, option = "C", end = .80) +
  labs(x = "Test", y = "Mean rating", color = "Triplet") +
  theme_clean()
```
...
sdtdata_4_agg_within |>
  ...
  scale_color_viridis_d(direction = -1, option = "C", end = .85) +
  ylim(0, 3) +
  labs(x = "", y = "Learning score", color = "Group") +
  theme_clean() +
  guides(color = guide_legend(
    title = "Group",
    title.position = "top"
...
FIrst, we create a data set by taking the mean rating for each person in each test for both old and new items. Then we compute the _learning score_ by subtracting the mean rating for new items from the mean rating for old items. 
...
```{r}
normalize <- function(x, max_x = 4) {
  min_x <- -max_x
  (x - min_x)/(max_x - min_x)
  }

dd <- dd |> 
  mutate(normscore = normalize(score))
```
...
fit_ls_2_groups <- brm(score ~ group2 * test + (1 | ID),
                      family = gaussian,
                      prior = priors,
                      data = sdtdata_2,
                      chains = 4, iter = 2000, cores = 4,
                      backend = "cmdstanr",
                      file = here::here("models/fit_ls_2_groups"),
                      save_model = here::here("stancode/fit_ls_2_groups.stan")
                      ) |>
  add_criterion("loo")
```
...
fit_ls_3_groups <- brm(score ~ group3 * test + (1 | ID),
                      family = gaussian,
                      prior = priors,
                      data = sdtdata_3,
                      chains = 4, iter = 2000, cores = 4,
                      backend = "cmdstanr",
                      file = here::here("models/fit_ls_3_groups"),
                      save_model = here::here("stancode/fit_ls_3_groups.stan")
                      ) |>
  add_criterion("loo")
```
## 3 groups (controls, musicians, synaesthetes)
...
fit_ls_3_groups_alt <- brm(score ~ group3alt * test + (1 | ID),
                      family = gaussian,
                      prior = priors,
                      data = sdtdata_3alt,
                      chains = 4, iter = 2000, cores = 4,
                      backend = "cmdstanr",
                      file = here::here("models/fit_ls_3_groups_alt"),
                      save_model = here::here("stancode/fit_ls_3_groups_alt.stan")
                      ) |>
  add_criterion("loo")
```
## 4 groups
...
fit_ls_4_groups <- brm(score ~ group4 * test + (1 | ID),
                      family = gaussian,
                      prior = priors,
                      data = dd,
                      chains = 4, iter = 2000, cores = 4,
                      backend = "cmdstanr",
                      file = here::here("models/fit_ls_4_groups"),
                      save_model = here::here("stancode/fit_ls_4_groups.stan")
                      ) |>
  add_criterion("loo")
```
...
```{r}
epred_ls_4_groups <- dd |>
  data_grid(group4, test) |>
  add_epred_draws(fit_ls_4_groups,
                  re_formula = ~ID,
                  ndraws = 500)
w```
...
```{r}
epred_ls_4_groups |>
  ggplot(aes(x = test, y = .epred, color = group4)) +
...
```{r}
epred_ls_4_groups_contrast <- 
  expand_grid(group4 = c("relpitch", "abspitch"), 
         test = levels(dd$test)) |> 
  add_epred_draws(fit_ls_4_groups,
                  re_formula = ~ID,
                  ndraws = 500)
...
```{r}
contrasts_rel_abs <- fit_ls_4_groups |> 
  emmeans::emmeans(~group4 | test) |> 
  emmeans::contrast("pairwise") |> 
  gather_emmeans_draws() |> 
  mutate(indicator = if_else(contrast == "relpitch - abspitch", 1, 0))
```



```{r}
zero_color <- ggokabeito::palette_okabe_ito()[1]

contrasts_rel_abs |> 
  ggplot(aes(y = contrast, x = .value, fill = as_factor(indicator))) +
...
The learning score is computed by averaging over individual ratings. We thus lose information about the subject- and group specific usage of individual ratings. Of particular interest are the extreme rating categories, which denote extreme certainty that either _new_ items have never been seen (rating 1) or that _old_ items have been previously seen (rating 5).  
...
d |>
  ...
  facet_wrap(~ID) +
  ylab("Proportion") +
  labs(x = "Triplet", y = "Proportion", color = "Rating") +
  theme_clean() +
...
```{r}
epred_2_groups |>
  ggplot(aes(x = oldnew, y = .epred, color = rating)) +
  stat_pointinterval(position = position_dodge(width = .4)) +
  ...
  # scale_color_manual(values = brewer.pal(6, "Blues")[-c(1)]) +
  scale_color_viridis_d(begin = 0.0, end = 0.8) +
  theme_clean()
```

```{r eval=FALSE, include=FALSE}
group2_plot <- epred_2_groups |> 
  ggplot(aes(x = .epred, y = rating)) +
  coord_cartesian(expand = FALSE) +
  facet_grid(test + group2 ~ oldnew, switch = "x", labeller = label_value) +
  theme_classic() +
  theme(strip.background = element_blank(), strip.placement = "outside") +
  ggtitle("P(rating | item, test, group)") +
...
```{r}
epred_3_groups |>
  ggplot(aes(x = rating, y = .epred, color = oldnew)) +
  stat_pointinterval(position = position_dodge(width = .4)) +
...
```{r}
contrasts_ordinal <- fit_4_groups |> 
  emmeans::emmeans(~group4 | test + oldnew) |> 
  emmeans::contrast("pairwise") |> 
  gather_emmeans_draws() |> 
  mutate(indicator = if_else(contrast == "relpitch - abspitch", 1, 0))
```
...
```{r}
posterior_expectations <- epred_4_groups |> 
  mutate(product = as.double(rating) * .epred) |> 
  # group and convert to the sum-score metric
  group_by(group4, test, oldnew, .draw) |> 
  summarise(mean_rating = sum(product)) |> 
  pivot_wider(names_from = oldnew, values_from = mean_rating) |> 
  mutate(score = old - new)
```
...
posterior_expectations |> 
  ...
  group_by(group4, test) |> 
  mean_qi(score) |> 
  ggplot(aes(y = score, x = test, color = group4, ymin = .lower, ymax = .upper)) +
  geom_line(aes(group = group4), linewidth = 1.5, linetype = "dashed") +
  geom_pointinterval() +
  scale_color_viridis_d(direction = -1, option = "C", end = .85) +
  ylab("Learning score") +
...
```{r}
newdata <- expand_grid(group3alt = c("syn", "musician"),
                       oldnew = c("new", "old"),
                       test = c("colors", "tones", "combined"), 
                       ID = NA) |> 
  mutate(across(where(is_character), as_factor),
         group3alt = fct_relevel(group3alt,
                                 "musician", "syn"),
         test = fct_relevel(test,
                            "colors", "tones", "combined"))

preds <- fit_3_groups_alt |> 
  add_epred_draws(newdata = newdata,
                  category = "rating",
                  dpar = TRUE,
                  re_formula = ~(1 | ID) + (1 | item),
                  ndraws = 500,
...
preds |> 
  ...
  facet_grid(test ~ group3alt, labeller = label_both) +
  scale_size_continuous(guide = FALSE) +
  expand_limits(y = 0) +
  # scale_color_okabe_ito() +
  scale_color_viridis_d(begin = 0.0, end = 0.8) +
  labs(x = "Rating", y = "Predicted probability", color = "Triplet") +
  theme_clean() +
...
```{r}
newdata <- expand_grid(group3alt = "syn",
                       oldnew = c("new", "old"),
                       test = "combined", 
                       ID = str_c("Synaesthete ", 1:10)) |> 
  mutate(across(where(is_character), as_factor),
         test = fct_relevel(test,
                            "colors", "tones", "combined"))
...
```{r}
newdata <- expand_grid(group3alt = "musician",
                       oldnew = c("new", "old"),
                       test = "combined", 
                       ID = str_c("Musician ", 1:10)) |> 
  mutate(across(where(is_character), as_factor),
         test = fct_relevel(test,
                            "colors", "tones", "combined"))
